...
class BaseEncoder(nn.Module):
    """
    Modified backbone pipeline for mmMobileViT_Track.
    """
    ...
    def _forward_MobileViT_layer(self, layer: nn.Module, x_v: Tensor, z_v: Tensor, x_i: Tensor, z_i: Tensor):

        num_blocks = len(layer)

        # the first block is always MobilenetV2 with down-sampling
        MobilenetV2_block = layer[0]
        x_v = MobilenetV2_block(x_v)
        z_v = MobilenetV2_block(z_v)
        x_i = MobilenetV2_block(x_i)
        z_i = MobilenetV2_block(z_i)

        # compute output for remaining Transformer blocks (i.e., MobileViT/MobileViT-v2)
        for i in range(1, num_blocks):
            block = layer[i]
            x_v, z_v, x_i, z_i = block(x_v, z_v, x_i, z_i)

        return x_v, z_v, x_i, z_i
    ...
    def forward(self, x: Tensor, z: Tensor):
        ...
        Returns:
            ...
            attn : None
        """
        x_v, z_v, x_i, z_i = self.forward_features(x, z,)
