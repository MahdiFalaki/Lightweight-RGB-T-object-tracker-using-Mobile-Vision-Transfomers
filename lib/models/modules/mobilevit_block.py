...
class FusionTransformer(MobileViTBlockv2):
    def __init__(
        self,
        opts,
        in_channels: int,
        ...
        self.final_fusion= WeightedFusion(128)

    def forward(self, x: Tensor, z: Tensor) -> Tensor:

        # convert feature map to patches
        if self.enable_coreml_compatible_fn:
            patches_x, output_size_x = self.unfolding_coreml(x)
            patches_z, output_size_z = self.unfolding_coreml(z)
        else:
            patches_x, output_size_x = self.unfolding_pytorch(x)
            patches_z, output_size_z = self.unfolding_pytorch(z)
        ...
        fused = self.final_fusion(fm_x, fm_z)
...
class WeightedFusion(nn.Module):
    """
    Weighted fusion of RGB and IR features using learnable channel-wise weights.
    """

    def __init__(self, channels, use_batchnorm=True):
        super(WeightedFusion, self).__init__()
        # Learnable weights for RGB and IR
        self.weight_rgb = nn.Parameter(torch.ones(1, channels, 1, 1))  # Shape: (1, C, 1, 1)
        self.weight_dte = nn.Parameter(torch.ones(1, channels, 1, 1))  # Shape: (1, C, 1, 1)
        self.sigmoid = nn.Sigmoid()  # To normalize weights between 0 and 1
    ...
    def forward(self, feat_rgb, feat_dte):
        """
        feat_rgb: Features from RGB modality.
        feat_dte: Features from IR modality.
        """
        # Compute normalized weights
        weight_rgb = self.sigmoid(self.weight_rgb)  # Shape: (1, C, 1, 1)
        weight_dte = self.sigmoid(self.weight_dte)  # Shape: (1, C, 1, 1)

        # Apply weights to the feature maps
        weighted_rgb = weight_rgb * feat_rgb  # Shape: (B, C, H, W)
        weighted_dte = weight_dte * feat_dte  # Shape: (B, C, H, W)

        # Combine the weighted features
        fused_features = weighted_rgb + weighted_dte  # Shape: (B, C, H, W)
